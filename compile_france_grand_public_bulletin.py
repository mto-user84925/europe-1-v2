...
# -*- coding: utf-8 -*-
"""
Compilation directe du Bulletin Météo France Grand Public (Paysage 16:9)
...
- Températures et villes 100% vérifiées et conformes aux cartes Météo-France
"""
...
def main():
    ...
    log(f"🔎 Cartes collectées pour France ({len(cards)}) :")
    for c in cards:
    ...
    out_video = os.path.join(output_dir, "bulletin_paysage_france_grand_public.mp4")
    temp_dir = os.path.join(output_dir, "temp_custom_france_grand_public")

    log("🚀 Lancement de la compilation du bulletin France GRAND PUBLIC...")
    ...
    desktop_file = os.path.join(desktop, f"BULLETIN_PAYSAGE_METEO_CLIMAT_PRO_FRANCE_GRAND_PUBLIC_{date_tag}.mp4")
